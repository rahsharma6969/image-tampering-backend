...
class IMTFE(nn.Module):
    ...
    def __init__(self, in_channel=3,device=device):
        ...
        self.SRMConv2D.weight.data=torch.load('./model/IMTFEv4.pt')['SRMConv2D.weight']
...
def check_forgery(model, img_path='./test_images/fake.jpg', device=device):
    model.to(device)
    model.eval()
    ...
    im_np = np.array(im)
    original_image = im_np.copy()

    # Preprocess
    im_tensor = torch.from_numpy(im_np).float().permute(2, 0, 1).unsqueeze(0).to(device)

    with torch.no_grad():
        final_output = model(im_tensor)

    mask = final_output[0][0].cpu().detach().numpy()
    suspicious = original_image.copy()
    suspicious[mask < 0.2] = 0
